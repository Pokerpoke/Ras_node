...
SYS_ROOT_DIR="/usr/arm-linux-gnueabi"
cd ${SCRIPT_DIR}
...
function git_cmake()
{
    ...
    mkdir -p cmake/toolschain && wget http://192.168.0.9/git/Pokerpoke/aero-node/raw/master/cmake/toolschain/arm.cmake -O ./cmake/toolschain/arm.cmake

    # insert cmake toolschain file path into CMakeLists.txt
    sed -i "1i \
    set(CMAKE_TOOLCHAIN_FILE ${PROJECT_DIR}/cmake/toolschain/arm.cmake)" CMakeLists.txt

    # build
    mkdir build && cd build && cmake -DCMAKE_INSTALL_PREFIX=${SYS_ROOT_DIR}/ ..
    if [ $? -ne 0 ]
    then
